...
public class RSSPullParser extends DefaultHandler {
    ...
    // An attribute value indicating that the element contains an item - flickr version
    private static final String ENTRY = "entry";

    // An attribute value indicating that the element contains a link - flickr version
    //in flickr, the link tag that has a first attribute named rel with a value of "enclosure"
    // has a 3rd attribute called href that has the image url in it
            ...
            // At the start of a tag, gets the tag's name
                ...
                // If this isn't an item, then checks for other options
                    ...
                    if (eventName.equalsIgnoreCase(CONTENT)) {
                        //if (localXmlPullParser.getAttributeValue(null, "rel").equalsIgnoreCase("enclosure")) {
                            ...
                            //urlValue = localXmlPullParser.getAttributeValue(null, "href");
                        //}
                    // If it's a THUMBNAIL
